...
class Products
{
	...
	public function getProducts(){
		...
		if($uname != 'storeadmin@tsalastore.co.za'){
			...
			$q = $this->con->query("SELECT p.product_id, p.product_title, p.product_price,p.product_qty, p.product_desc, p.color, p.product_size, p.product_image, p.new_fashion,p.special_offer,p.special_offer,p.on_sale,p.clearance,p.item_type, c.cat_title, c.cat_id, b.brand_id, b.brand_title FROM products p JOIN categories c ON c.cat_id = p.product_cat JOIN brands b ON b.brand_id = p.product_brand WHERE p.vendor_name= '$uname'");
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/big_store/web/images/".$uniqueImageName)) {
					...
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`,`product_title`,`product_desc`,`color`,`product_size`,`product_qty`,`product_price`, `vendor_name`,`new_fashion`,`special_offer`,`on_sale`,`clearance`,`item_type`,`product_image`
					) VALUES ('$category_id', '$brand_id', '$product_name', '$product_desc','$color','$product_size','$product_qty', '$product_price', '$uname','$new_fashion','$special_offer','$on_sale','$clearance','$item_type','$uniqueImageName')");
...
//	&& !empty($color)
	...
	&& !empty($_FILES['product_image']['name'])) {
        ...
        //'$uname','$new_fashion','$special_offer','$on_sale','$clearance','$item_type','$uniqueImageName'
		$p = new Products();
		$result = $p->addProduct($category_id,
								$brand_id,
								$product_name,
								$product_desc,
								$color,
